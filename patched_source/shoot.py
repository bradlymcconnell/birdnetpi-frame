...
def shoot(url, out, *, title=None, subtitle=None, vw=600, vh=800, dsf=2,
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if bird_names:
                try:
                    page.evaluate("async () => { try { await document.fonts.load('600 16px Hand'); await document.fonts.ready; } catch(e) {} }")
                except Exception:
                    pass
            ...
            if page.query_selector(".gtile") is not None:
                ...
                if bird_names:
                    ...
                    if missing_labels:
                        print("frame labels missing for: " + ", ".join(missing_labels), file=sys.stderr)
